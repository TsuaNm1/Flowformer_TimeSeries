...
def myspace():
    # timestamps = np.array(['2021-01-01', '2021-01-02', '2021-01-03'], dtype='datetime64')
    # values = np.array([10, 20, 30])
    #
    # # 合并时间戳和值
    # ts = np.column_stack((timestamps, values))
    #
    # # 保存为.ts文件
    # np.savetxt('timeseries.ts', ts, delimiter='\t', fmt='%s')



    # # 读取.ts文件
    # data = np.genfromtxt('data.ts', delimiter='\t', dtype=None, names=('timestamp', 'value'))
    #
    # # 可选：处理时间戳数据
    ...
    # print(timestamps)
    #
    # data2 = np.genfromtxt('Heartbeat/Heartbeat_TRAIN.ts', delimiter='\t', dtype=None, names=('timestamp', 'value'))
    #
    # # 可选：处理时间戳数据
    # timestamps1 = data2['timestamp']
    #
    # # 打印读取的时间序列数据
    # print(timestamps1)

    data = np.array([9.49E-4, 0.001488, 3.14E-4, 9.95E-4, 0.002099, 0.001732, 0.002043, 9.55E-4, 3.79E-4, 0.001024,
                     0.001063, 6.73E-4, 6.85E-4, 6.79E-4, 0.001264, 8.59E-4, 0.001092, 0.001199, 0.001487, 5.14E-4,
                     5.29E-4, 6.83E-4, 0.001843, 0.001117, 0.001168, 8.34E-4, 0.001059, 7.12E-4, 2.82E-4, 7.34E-4,
                     0.00128, 8.31E-4, 3.77E-4, 2.67E-4, 7.43E-4, 0.001036, 3.79E-4, 8.92E-4, 8.02E-4, 4.47E-4,
                     9.92E-4, 0.001468, 0.001366, 0.001546, 0.002022, 0.001709, 0.001096, 0.001271, 0.001042,
                     0.001687, 0.0213])

    #创建.ts文件内容字符串
    ts_content = "@problemName FetalHeartRate\n" \
                 "@timeStamps false\n" \
                 "@missing false\n" \
                 "@univariate false\n" \
                 "@dimensions 2\n" \
                 "@equalLength true\n" \
                 "@seriesLength 1200\n" \
                 "@classLabel true early late normal prolong variety\n" \
                 "@data\n".format(data.size)

    # 添加数据到.ts文件内容字符串
    ts_content += ",".join([str(val) for val in data]) + ":abnormal"
